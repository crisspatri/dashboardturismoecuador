...
// ############# TARTA ###################
	...
	function dsPieChart6(){
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/6.EntExtr%20segun%20genero.csv', function(data) {
			...
			var refresh=d3.select("#idGenero")				
				.on("mouseover", mouseoverRefresh)
				.on("mouseout", mouseoutRefresh)
				.on("click", up6Refresh);

			var vis = d3.select("#pieChart6")
			...
			function up6(d, i) {
				updateBarChart6(d.data.key, color(i));
			} 
			...
			function up6Refresh(d, i) {
				d3.select("#pieChart6 svg").remove();
				d3.select("#barChart6 svg").remove();
				d3.select("#lineChart6 svg").remove();
				dsPieChart6();
				dsBarChart6();
				dsLineChart6();
			} 
...
// ############# BARRAS ###################
	var datasetBarChart6 = [];

	function datasetBarChosen6(anio) {
		var ds = [];
		datasetBarChart6.forEach(function(d,i){
			if (d.ANIO==anio)
				ds.push(datasetBarChart6[i]);			
		});

		return ds;
	}

	function dsBarChartBasics6() {
	...
	function dsBarChart6() {

		var basics = dsBarChartBasics6();
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/6.EntExtr%20segun%20genero.csv', function(dataBC) {

			datasetBarChart6=dataBC;
	...
	function updateBarChart6(anio, colorChosen) {
		
		var currentDatasetBarChart = datasetBarChosen6(anio);
		...
		var svg = d3.select("#barChart6 svg");
	      
		var plot = d3.select("#barChartPlot6")
...
// ############# LINE CHART ###################
	var datasetLineChart6 = [];

	function datasetLineChartChosen6(continente) {
		var ds = [];
		datasetLineChart6.forEach(function(d,i){
				if (d.GENERO==continente)
					ds.push(datasetLineChart6[i]);			
		});
	...
	function dsLineChart6() {
		...
		var margin = basics.margin,
	   	...
	   	d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/6.EntExtr%20segun%20genero.csv', function(dataLC) {

			datasetLineChart6=dataLC;
	...
	function updateLineChart6(continente, colorChosen) {

		var currentDatasetLineChart = datasetLineChartChosen6(continente);   
		var basics = dsLineChartBasics();
		...
		var plot = d3.select("#lineChartPlot6")
