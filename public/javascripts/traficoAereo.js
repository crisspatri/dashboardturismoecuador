...
// ############# TARTA ###################
	...
	function dsPieChart8(){
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/8.Trafico%20aereo%20domestico%20regular%20pasajeros2015.csv', function(data) {

			data =d3.nest()
				.key(function(d) { return d.MESES; })
			...
			var refresh=d3.select("#idTraficoAereo")				
				.on("mouseover", mouseoverRefresh)
				.on("mouseout", mouseoutRefresh)
				.on("click", up8Refresh);

			var vis = d3.select("#pieChart8")
			...
			function up8(d, i) {
				updateBarChart8(d.data.key, color(i));
			} 
			...
			function up8Refresh(d, i) {
				d3.select("#pieChart8 svg").remove();
				d3.select("#barChart8 svg").remove();
				d3.select("#lineChart8 svg").remove();
				dsPieChart8();
				dsBarChart8();
				dsLineChart8();
			} 
...
// ############# BARRAS ###################
	var datasetBarChart8 = [];

	function datasetBarChosen8(anio) {
		var ds = [];
		datasetBarChart8.forEach(function(d,i){
			if (d.MESES==anio)
				ds.push(datasetBarChart8[i]);			
		});
	...
	function dsBarChart8() {
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/8.Trafico%20aereo%20domestico%20regular%20pasajeros2015.csv', function(dataBC) {

			datasetBarChart8=dataBC;
			
			dataBC =d3.nest()
				.key(function(d) { return d.RUTA; })
				.rollup(function(v) { return {
	    			count: v.length,
	    			total: d3.sum(v, function(d) { return d.ENTRADAS; })
	    		};})
	...
	function updateBarChart8(anio, colorChosen) {
		
		var currentDatasetBarChart1 = datasetBarChosen8(anio);
		...
		var svg = d3.select("#barChart8 svg");
	      
		var plot = d3.select("#barChartPlot8")
...
// ############# LINE CHART ###################
	var datasetLineChart8 = [];

	function datasetLineChartChosen8(continente) {
		var ds = [];
		datasetLineChart8.forEach(function(d,i){
				if (d.RUTA==continente)
					ds.push(datasetLineChart8[i]);			
		});
	...
	function dsLineChart8() {
		...
		var margin = basics.margin,
	   	...
	   	d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/8.Trafico%20aereo%20domestico%20regular%20pasajeros2015.csv', function(dataLC) {

			datasetLineChart8=dataLC;
	...
	function updateLineChart8(continente, colorChosen) {

		var currentDatasetLineChart = datasetLineChartChosen8(continente);   
		var basics = dsLineChartBasics();
		...
		var plot = d3.select("#lineChartPlot8")
