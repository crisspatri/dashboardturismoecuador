...
// ############# TARTA ###################
	...
	function dsPieChart5(){
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/5.EntExtr%20por%20grupos%20de%20edad.csv', function(data) {
			...
			var refresh=d3.select("#idEdades")				
				.on("mouseover", mouseoverRefresh)
				.on("mouseout", mouseoutRefresh)
				.on("click", up5Refresh);

			var vis = d3.select("#pieChart5")
			...
			function up5(d, i) {
				updateBarChart5(d.data.key, color(i));
			} 
			...
			function up5Refresh(d, i) {
				d3.select("#pieChart5 svg").remove();
				d3.select("#barChart5 svg").remove();
				d3.select("#lineChart5 svg").remove();
				dsPieChart5();
				dsBarChart5();
				dsLineChart5();
			} 
...
// ############# BARRAS ###################
	var datasetBarChart5 = [];

	function datasetBarChosen5(anio) {
		var ds = [];
		datasetBarChart5.forEach(function(d,i){
			if (d.ANIO==anio)
				ds.push(datasetBarChart5[i]);			
		});
	...
	function dsBarChart5() {
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/5.EntExtr%20por%20grupos%20de%20edad.csv', function(dataBC) {

			datasetBarChart5=dataBC;
			
			dataBC =d3.nest()
				.key(function(d) { return d.EDADES; })
				.rollup(function(v) { return {
	    			count: v.length,
	    			total: d3.sum(v, function(d) { return d.ENTRADAS; })
	    		};})
	...
	function updateBarChart5(anio, colorChosen) {
		
		var currentDatasetBarChart = datasetBarChosen5(anio);
		...
		var svg = d3.select("#barChart5 svg");
	      
		var plot = d3.select("#barChartPlot5")
...
// ############# LINE CHART ###################
	var datasetLineChart5 = [];

	function datasetLineChartChosen5(continente) {
		var ds = [];
		datasetLineChart5.forEach(function(d,i){
				if (d.EDADES==continente)
					ds.push(datasetLineChart5[i]);			
		});
	...
	function dsLineChart5() {
		...
		var margin = basics.margin,
	   	...
	   	d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/5.EntExtr%20por%20grupos%20de%20edad.csv', function(dataLC) {

			datasetLineChart5=dataLC;
	...
	function updateLineChart5(continente, colorChosen) {

		var currentDatasetLineChart = datasetLineChartChosen5(continente);   
		var basics = dsLineChartBasics();
		...
		var plot = d3.select("#lineChartPlot5")
