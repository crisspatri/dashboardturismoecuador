...
// ############# TARTA ###################
	...
	function dsPieChart12(){
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/12.Establecimiento%20alimentos%20y%20bebidas%20por%20provincias2015.csv', function(data) {

			data =d3.nest()
				.key(function(d) { return d.OBJETOS; })
				.rollup(function(v) { return {
	    			count: v.length,
	    			total: d3.sum(v, function(d) { return d.CANTIDAD; })
	    		};})
				.entries(data);
				console.log(JSON.stringify(data));

			var refresh=d3.select("#idEstablecimientos")				
				.on("mouseover", mouseoverRefresh)
				.on("mouseout", mouseoutRefresh)
				.on("click", up12Refresh);

			var vis = d3.select("#pieChart12")
			...
			function up12(d, i) {
				updateBarChart12(d.data.key, color(i));
			} 
			...
			function up12Refresh(d, i) {
				d3.select("#pieChart12 svg").remove();
				d3.select("#barChart12 svg").remove();
				dsPieChart12();
				dsBarChart12();
			} 
...
// ############# BARRAS ###################
	var datasetBarChart12 = [];

	function datasetBarChosen12(anio) {
		var ds = [];
		datasetBarChart12.forEach(function(d,i){
			if (d.OBJETOS==anio)
				ds.push(datasetBarChart12[i]);			
		});

		return ds;
	}

	function dsBarChartBasics12() {
	...
	function dsBarChart12() {

		var basics = dsBarChartBasics12();
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/12.Establecimiento%20alimentos%20y%20bebidas%20por%20provincias2015.csv', function(dataBC) {

			datasetBarChart12=dataBC;
	...
	function updateBarChart12(anio, colorChosen) {
		
		var currentDatasetBarChart1 = datasetBarChosen12(anio);
		...
		var svg = d3.select("#barChart12 svg");
	      
		var plot = d3.select("#barChartPlot12")
