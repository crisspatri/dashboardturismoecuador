...
// ############# TARTA ###################
	...
	function dsPieChart4(){
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/4.EntExtr%20por%20motivo%20de%20viaje%20segun%20ocupaciones2015.csv', function(data) {

			data =d3.nest()
				.key(function(d) { return d.MOTIVO; })
			...
			var refresh=d3.select("#idMotivo")				
				.on("mouseover", mouseoverRefresh)
				.on("mouseout", mouseoutRefresh)
				.on("click", up4Refresh);

			var vis = d3.select("#pieChart4")
			...
			function up4(d, i) {
				updateBarChart4(d.data.key, color(i));
			} 
			...
			function up4Refresh(d, i) {
				d3.select("#pieChart4 svg").remove();
				d3.select("#barChart4 svg").remove();
				dsPieChart4();
				dsBarChart4();
			} 
...
// ############# BARRAS ###################
	var datasetBarChart4 = [];

	function datasetBarChosen4(anio) {
		var ds = [];
		datasetBarChart4.forEach(function(d,i){
			if (d.MOTIVO==anio)
				ds.push(datasetBarChart4[i]);			
		});

		return ds;
	}

	function dsBarChartBasics4() {
	...
	function dsBarChart4() {

		var basics = dsBarChartBasics4();
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/4.EntExtr%20por%20motivo%20de%20viaje%20segun%20ocupaciones2015.csv', function(dataBC) {

			datasetBarChart4=dataBC;
			
			dataBC =d3.nest()
				.key(function(d) { return d.OCUPACIONES; })
				.rollup(function(v) { return {
	    			count: v.length,
	    			total: d3.sum(v, function(d) { return d.ENTRADAS; })
	    		};})
			...
			xLabels.selectAll("text.xAxis")
				...
				.attr("y", 15)
				.attr("class", "xAxis")
				.call(wrap, width / firstDatasetBarChart.length - barPadding)
				.append("title")
				.text(function(d) { return d.key;});	
	...
	function updateBarChart4(anio, colorChosen) {
		
		var currentDatasetBarChart = datasetBarChosen4(anio);
		...
		var svg = d3.select("#barChart4 svg");
	      
		var plot = d3.select("#barChartPlot4")			
		...
		xLabels.selectAll("text.xAxis")
			...
			.attr("y", 15)
			.attr("class", "xAxis")
			.call(wrap, width / currentDatasetBarChart.length - barPadding)
			.append("title")
			.text(function(d) { return d.key;});	
