...
// ############# TARTA ###################
	...
	function dsPieChart9(){
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/9.Trafico%20vial2015.csv', function(data) {

			data =d3.nest()
				.key(function(d) { return d.VEHICULO; })
			...
			var refresh=d3.select("#idTraficoVial")				
				.on("mouseover", mouseoverRefresh)
				.on("mouseout", mouseoutRefresh)
				.on("click", up9Refresh);

			var vis = d3.select("#pieChart9")
			...
			function up9(d, i) {
				updateBarChart9(d.data.key, color(i));
			} 
			...
			function up9Refresh(d, i) {
				d3.select("#pieChart9 svg").remove();
				d3.select("#barChart9 svg").remove();
				d3.select("#lineChart9 svg").remove();
				dsPieChart9();
				dsBarChart9();
				dsLineChart9();
			} 
...
// ############# BARRAS ###################
	var datasetBarChart9 = [];

	function datasetBarChosen9(anio) {
		var ds = [];
		datasetBarChart9.forEach(function(d,i){
			if (d.VEHICULO==anio)
				ds.push(datasetBarChart9[i]);			
		});

		return ds;
	}

	function dsBarChartBasics9() {
	...
	function dsBarChart9() {

		var basics = dsBarChartBasics9();
		...
		d3.csv('https://raw.githubusercontent.com/crisspatri/dashboardturismoecuador/master/public/data/9.Trafico%20vial2015.csv', function(dataBC) {

			datasetBarChart9=dataBC;
			
			dataBC =d3.nest()
				.key(function(d) { return d.TRAMO; })
				.rollup(function(v) { return {
	    			count: v.length,
	    			total: d3.sum(v, function(d) { return d.ENTRADAS; })
	    		};})
	...
	function updateBarChart9(anio, colorChosen) {
		
		var currentDatasetBarChart1 = datasetBarChosen9(anio);
		...
		var svg = d3.select("#barChart9 svg");
	      
		var plot = d3.select("#barChartPlot9")
